...
| 영역      | 학생                                                | 선생님                                              | 공통 기반                         |
| --------- | --------------------------------------------------- | --------------------------------------------------- | --------------------------------- |
| 클래스    | 초대 코드로 클래스 참여                             | 클래스 생성·초대 코드 관리                          | 역할별 접근 제어                  |
| 과제      | 긴급·이번 주·나중 필터, 즐겨찾기, 완료 처리         | 과제 CRUD, 수행평가·시험·준비물 유형, 평가기준 편집 | PostgreSQL·Prisma 영속화          |
...
- **AI 과제 도우미**: 과목·제목·설명·평가기준·마감일·학생 프로필·현재 한국 날짜·남은 날짜를 서버에서 구성합니다. 최근 16개 메시지만 문맥으로 전달하고 전체 대화는 과제별로 저장합니다.
- **사용량 제한**: 한국 시간 자정 기준 FREE 10회, PRO 100회입니다. 추가로 인증은 이메일별 15분 10회·전체 300회, 사용자 쓰기는 분당 100회, AI 요청은 분당 6회로 제한합니다. 요청 전에 DB에서 원자적으로 예약하고 제공자 오류가 나면 횟수를 복구합니다.
- **첨부파일**: PDF·PNG·JPEG·TXT를 파일당 최대 5MiB까지 저장합니다. 다운로드 전에 클래스 소속과 과제 접근 권한을 확인하며 파일 내용 자동 분석은 아직 제공하지 않습니다.
- **제출·검토**: 학생은 텍스트 결과를 제출하고, 선생님은 `수정 요청` 또는 `검토 완료`와 피드백을 기록합니다. 학생의 완료와 선생님의 검토 완료는 서로 다른 상태입니다.
...
| `AI_BASE_URL`  | AI 사용 시 | `/chat/completions` 앞까지의 URL. 예: `https://api.openai.com/v1`                                            |
| `AI_MODEL`     | AI 사용 시 | 제공자가 지원하는 모델 ID                                                                                    |

AI를 연결하려면 위 AI 변수 3개를 설정하고 서버를 재시작한 뒤 학생 과제 상세에서 질문합니다. 서버는 `POST {AI_BASE_URL}/chat/completions`로 `{ model, messages, max_tokens: 1600 }`을 보내는 비스트리밍 어댑터를 사용합니다. 다른 응답 형식은 [AI 어댑터](src/lib/ai.ts)에서 조정합니다.

학생의 질문에는 이름·학교·학년·반, 과제 문맥, 최근 대화가 포함되어 선택한 외부 AI 제공자로 전송될 수 있습니다. 이메일·비밀번호·세션은 보내지 않습니다. 학교에서 공개 운영할 때는 데이터 전달·보관 방침과 필요한 동의 절차를 마련하세요.

`.env`와 API 키는 절대 커밋하지 마세요. AI 설정이 비어 있어도 클래스·과제·제출 기능은 동작하며 AI 영역에는 연결 필요 상태가 표시됩니다.

## 개발용 샘플 계정

`npm run db:seed`는 운영 환경(`NODE_ENV=production`)에서 실행되지 않습니다. 아래 계정은 로컬 시연 전용입니다.
...
플랜을 변경하는 로그인 사용자용 API나 결제 연동은 없습니다. 개발환경에서만 `npm run dev:plan -- student@example.com PRO` 또는 `npm run dev:plan -- student@example.com FREE`로 플랜별 화면과 한도를 확인할 수 있습니다.
...
E2E는 `55433` 포트의 별도 PostgreSQL(`.local/e2e-postgres`), `4318` 포트의 결정적 로컬 AI 제공자, `3100` 포트의 빌드된 Next.js 서버를 사용합니다. 실제 운영 DB나 외부 AI 키를 호출하지 않으며 `npm run build`를 먼저 실행해야 합니다.
...
- 모든 서버 작업에서 역할·클래스 소속·소유권을 검사하고, Server Actions와 AI API에서 Origin을 확인합니다.
- Zod 서버 검증, Prisma 매개변수 쿼리, React 텍스트 렌더링을 사용합니다. 사용자 HTML을 실행하지 않습니다.
- 첨부파일은 DB에 저장하고 `application/octet-stream`·`nosniff`로 다운로드합니다. 악성코드 검사는 아직 제공하지 않습니다.
- 인증·쓰기·AI 요청에 DB 기반 제한을 적용합니다. 운영에서는 프록시의 IP별 제한, 모니터링, 백업, 파일 총용량 제한도 추가하세요.
- 이메일 인증·비밀번호 재설정·계정 삭제·교사 재직 인증은 아직 제공하지 않습니다. 실제 학교 운영에서는 승인 절차와 개인정보 보관·삭제 정책을 별도로 설계해야 합니다.
